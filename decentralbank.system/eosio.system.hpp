...
namespace eosiosystem {
   ...
   struct [[eosio::table("global"), eosio::contract("eosio.system")]] eosio_global_state : eosio::blockchain_parameters {
      ...
      /// the global state table needs price feeds for the assets within the network so that prices
      /// the global state table is updated per block to update the average price. New price is reported each block
      int64_t              currency_price_ema; /// the exponential moving average price of the currency token
      int64_t              dmb_price_ema; /// the exponential moving average price of the maturity bond token
      int64_t              sss_price_ema; /// the exponential moving average price of the seigniorage share token
